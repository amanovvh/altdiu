...
type Category = 'OLYMPIAD' | 'CERTIFICATE' | 'COMPETITION' | 'SPORT' | 'OTHER';
...
export function AchievementForm({ action, initial = {}, submitLabel = 'Сохранить' }: Props) {
  ...
  const addTranslation = () => {
    ...
    if (next) setTranslations([...translations, { locale: next.value, title: '', description: '' }]);
  };
          ...
          {translations.map((t, idx) => (
            <div key={`${t.locale}-${idx}`} className="rounded-xl border border-ink-100 bg-surface-alt/40 p-5">
              ...
              <div className="space-y-4">
                <AdminInput label="Заголовок" name={`title_${t.locale}`} required
                  defaultValue={t.title}
                  onChange={(e) => updateTranslation(idx, 'title', (e.target as HTMLInputElement).value)} />
                <AdminTextarea label="Описание" name={`description_${t.locale}`}
                  defaultValue={t.description} rows={3}
      ...
      <div className="card p-6">
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <AdminSelect label="Категория" name="category"
            defaultValue={initial.category ?? 'OLYMPIAD'}
            options={CATEGORIES} />
          <AdminInput label="Дата" name="date" type="date" required
            defaultValue={initial.date ?? new Date().toISOString().slice(0, 10)} />
          <div className="md:col-span-2">
            <FileUploadField
              label="Фото"
              name="image"
              initialPublicId={initial.image ?? null}
              folder="lyceum/achievements"
            />
          </div>
        ...
        <div className="mt-6">
          <MultiImageUploadField
            ...
            folder="lyceum/achievements/gallery"
            hint="Дополнительные фото — грамоты, снимки с мероприятий, репортажи. Открываются на весь экран на публичной странице."
          />
        </div>
      </div>
